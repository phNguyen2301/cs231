...
def gen_stroke_map(img,
    ...
    for d in range(num_of_directions):
        C[:, :, d] = G * (max_pixel_indices_map == d
                          )  # (max_pixel_indices_map == d) is a binary matrix
    ...
    S_tag_normalized = (S_tag - np.min(S_tag.ravel())) / (
        np.max(S_tag.ravel()) - np.min(S_tag.ravel()))
...
def gen_tone_map(img, w_group=0):
    ...
    w_mat = np.array([[11, 37, 52], [29, 29, 42], [2, 22, 76]])
    w = w_mat[w_group, :]
    ...
    for v in range(num_pixel_vals):
        ...
        if (u_a <= v <= u_b):
            ...
            p2 = 0
        p3 = (1 / np.sqrt(2 * np.pi * sigma_d)) * np.exp(
            (-np.square(v - mu_d)) / (2 * np.square(sigma_d)))
    ...
    J = lut_normalized[(255 * img).astype(np.uint)]
    # smooth:
...
def gen_pencil_texture(img, H, J):
    ...
    logH_sparse = sparse.spdiags(logH.ravel(), 0, height * width,
                                 height * width)  # 0 - from main diagonal
...
def gen_pencil_drawing(img,
    ...
    if not rgb:
        ...
        yuv_img[:, :, 0] = R
        return exposure.rescale_intensity(color.yuv2rgb(yuv_img),
                                          in_range=(0, 1))
